...
#include "Trigger_fmu2_model_defines.h"
...
static int calc_initialize(component_ptr_t comp)
{
	...
	if(comp->loggingOn) {
		...
		comp->functions->logger(comp->functions->componentEnvironment, comp->instanceName, fmi2OK, "INFO", "Init #i%d#=%i", VAR_I_integer_cnt, comp->integers	[VAR_I_integer_cnt]);
		comp->functions->logger(comp->functions->componentEnvironment, comp->instanceName, fmi2OK, "INFO", "Init #i%d#=%i", VAR_I_integer_stop, comp->integers	[VAR_I_integer_stop]);
		comp->functions->logger(comp->functions->componentEnvironment, comp->instanceName, fmi2OK, "INFO", "Init #r%d#=%g", VAR_R_time_increment,comp->reals	[VAR_R_time_increment]);
		comp->functions->logger(comp->functions->componentEnvironment, comp->instanceName, fmi2OK, "INFO", "Init #r%d#=%g", VAR_R_state_init, comp->reals		[VAR_R_state_init]);
		comp->functions->logger(comp->functions->componentEnvironment, comp->instanceName, fmi2OK, "INFO", "Init #r%d#=%g", VAR_R_state_derivative, comp->reals[VAR_R_state_derivative]);
	}
...
{	
	...
	if (comp->states[VAR_R_state] < 0) {
		comp->states[VAR_R_state] =  comp->reals[VAR_R_state_init];
		if(comp->loggingOn) {
			comp->functions->logger(comp->functions->componentEnvironment, comp->instanceName, fmi2OK, "INFO", "Reset #r%d#=%g at %f", VAR_R_state, comp->states[VAR_R_state], comp->fmitime);
		}
	...
	if(comp->eventInfo.nextEventTime <= comp->fmitime) {
		...
		if (comp->integers	[VAR_I_integer_cnt] == comp->integers[VAR_I_integer_stop]){ 
			...
			if(comp->loggingOn) {
				comp->functions->logger(comp->functions->componentEnvironment, comp->instanceName, fmi2OK, "INFO", "Setting TimeEvent at %f to %f", comp->fmitime,comp->eventInfo.nextEventTime);
			}
		}
	}
