...
{
    fmi2_import_variable_typedef_t* td;
    ...
    for (i = 0; i < n_tds; i++) {
        td = fmi2_import_get_typedef(tds, i);
        tname = fmi2_import_get_type_name(td);
        if (strncmp(tname, tdPrefix, strlen(tdPrefix)) == 0) { /* only test the minimal tds, (test XML might contain others) */
            ASSERT_MSG(fmi2_import_get_type_quantity(td) == NULL, "expected default quantity to be null ptr");
            nTdsTested++;
        }
    }
