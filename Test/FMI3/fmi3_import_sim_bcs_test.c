...
int test_simulate_cs(fmi3_import_t * fmu)
{
	...
	jmstatus = fmi3_import_instantiate_basic_co_simulation(
            fmu,
            instanceName,
            resourceLocation,
            visible,
	...
	if (jmstatus == jm_status_error) {
		printf("fmi3_import_instantiate_basic_co_simulation failed\n");
		do_exit(CTEST_RETURN_FAIL);
	}
	...
	while (tcur < tend) {
		...
		fmistatus = fmi3_import_do_step(fmu, tcur, hstep, newStep, &terminate, &earlyReturn, &lastSuccessfulTime);
...
int main(int argc, char *argv[])
{
	...
	if (!(fmi3_import_get_fmu_kind(fmu) & fmi3_fmu_kind_bcs)) {
		printf("Only BCS 3.0 is supported by this code\n");
		do_exit(CTEST_RETURN_FAIL);
	}
	...
	status = fmi3_import_create_dllfmu(fmu, fmi3_fmu_kind_bcs, &instEnv, fmi3_test_log_forwarding_wrap);
