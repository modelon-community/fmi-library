...
void do_event_iteration(fmi3_import_t *fmu, test_event_info_t* eventInfo)
{
    ...
    while (eventInfo->newDiscreteStatesNeeded && !eventInfo->terminateSimulation) {
        fmi3_import_new_discrete_states(
                fmu,
...
{	
	...
	jmstatus = fmi3_import_instantiate_model_exchange(fmu, "Test ME model instance", NULL, fmi3_false);
...
{	
	...
	while ((tcur < tend) && (!(eventInfo.terminateSimulation || terminateSimulation))) {
		...
		if (step_event || zero_crossing_event || time_event) {
            fmistatus = fmi3_import_enter_event_mode(fmu, input_event, step_event, roots_found, n_event_indicators,
                    time_event);
            do_event_iteration(fmu, &eventInfo);
