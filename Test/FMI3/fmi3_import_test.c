...
static void fmi3logger(fmi3_component_environment_t env, fmi3_string_t instanceName, fmi3_status_t status, fmi3_string_t category, fmi3_string_t message, ...)
{
    int len;
	char msg[BUFFER];
	va_list argp;	
...
static void stepFinished(fmi3_component_environment_t env, fmi3_status_t status) {
...
int fmi3_test(fmi_import_context_t* context, const char* dirPath)
{
	...
	callBackFunctions.logger = fmi3logger;
	...
	else if(fmi3_import_get_fmu_kind(fmu) != fmi3_fmu_kind_me) {
