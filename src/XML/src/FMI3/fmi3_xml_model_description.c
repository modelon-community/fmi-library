...
int fmi3_xml_handle_fmiModelDescription(fmi3_xml_parser_context_t *context, const char* data) {
    ...
    if(!data) {
		...
		/* TODO: add checks for HCS and SCS when parsing of these model identifiers has been impl. */
		if( (md->fmuKind & fmi3_fmu_kind_bcs) && !fmi3_xml_is_valid_model_ID(fmi3_xml_get_model_identifier_CS(md))) {
...
int fmi3_xml_handle_CoSimulation(fmi3_xml_parser_context_t *context, const char* data) {
    ...
    if(!data) {
        ...
        /* update fmuKind */
        if (md->fmuKind == fmi3_fmu_kind_unknown) {
            md->fmuKind = fmi3_fmu_kind_bcs;
        } else {
            /* add BCS to found kinds (i.e. for multi-types) */
            md->fmuKind |= fmi3_fmu_kind_bcs;
        }
