...
fmi3_xml_model_description_t * fmi3_xml_allocate_model_description( jm_callbacks* callbacks) {
    ...
    md->defaultExperimentTolerance = FMI3_DEFAULT_EXPERIMENT_TOLERANCE;

    md->defaultExperimentStepSize = FMI3_DEFAULT_EXPERIMENT_STEPSIZE;
...
int fmi3_xml_handle_fmiModelDescription(fmi3_xml_parser_context_t *context, const char* data) {
    ...
    if(!data) {
		...
		ret =        /* <xs:attribute name="fmiVersion" type="xs:normalizedString" use="required" fixed="2.0"/> */
                    ...
                    fmi3_xml_set_attr_uint(context, fmi3_xml_elmID_fmiModelDescription, fmi_attr_id_numberOfEventIndicators, 0, &numEventIndicators,0);
		...
		if(md->fmuKind == fmi3_fmu_kind_unknown) {
			fmi3_xml_parse_fatal(context, "Neither ModelExchange nor CoSimulation element were parsed correctly. FMU kind not known.");
			return -1;
		}
		if( (md->fmuKind != fmi3_fmu_kind_cs) && !fmi3_xml_is_valid_model_ID(fmi3_xml_get_model_identifier_ME(md))) {
		...
		if( (md->fmuKind != fmi3_fmu_kind_me) && !fmi3_xml_is_valid_model_ID(fmi3_xml_get_model_identifier_CS(md))) {
		...
		if( (md->fmuKind == fmi3_fmu_kind_me_and_cs) && (strcmp(fmi3_xml_get_model_identifier_CS(md), fmi3_xml_get_model_identifier_ME(md)) == 0)) {
...
int fmi3_xml_handle_CoSimulation(fmi3_xml_parser_context_t *context, const char* data) {
    ...
    if(!data) {
		...
		if(md->fmuKind == fmi3_fmu_kind_me)
			md->fmuKind = fmi3_fmu_kind_me_and_cs;
		else
        ...
        return (	/* <xs:attribute name="modelIdentifier" type="xs:normalizedString" use="required"> */
                    ...
                    fmi3_xml_set_attr_uint(context,fmi3_xml_elmID_CoSimulation, fmi_attr_id_maxOutputDerivativeOrder,0,
