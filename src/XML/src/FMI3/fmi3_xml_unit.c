...
fmi3_xml_display_unit_t* fmi3_xml_get_unit_display_unit(fmi3_xml_unit_t* u, size_t index) {
    if(index >= fmi3_xml_get_unit_display_unit_number(u)) return 0;
...
double fmi3_xml_convert_from_display_unit(double val, fmi3_xml_display_unit_t* du, int isRelativeQuantity) {
    double factor = fmi3_xml_get_display_unit_factor(du);
    double offset = fmi3_xml_get_display_unit_offset(du);
    if(isRelativeQuantity)
        return val/factor;
    else
...
#define FMI3_PARSE_SI_BASE_UNIT_ENU(c) \
	fmi3_xml_set_attr_int32(context, fmi3_xml_elmID_BaseUnit, fmi_attr_id_ ## c, 0, &(unit->SI_base_unit_exp[fmi3_SI_base_unit_ ## c]), 0) ||
			ret =     	
...
int fmi3_xml_handle_Unit(fmi3_xml_parser_context_t *context, const char* data) {
    if(!data) {
            ...
            if( 
                ...
                fmi3_xml_set_attr_string(context, fmi3_xml_elmID_BaseUnit, fmi_attr_id_name, 1, buf) ||
...
int fmi3_xml_handle_DisplayUnit(fmi3_xml_parser_context_t *context, const char* data) {
    if(!data) {
            ...
            /*  <xs:attribute name="name" type="xs:normalizedString" use="required"/> */
            ret = fmi3_xml_set_attr_string(context, fmi3_xml_elmID_DisplayUnit, fmi_attr_id_name, 1, buf);
            if(ret) return ret;
